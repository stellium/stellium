...
function triggerHotReload() {
	...
	if (window.Stellium.DEV) console.log("%cHot Content Reloaded", "color:#F06E7B;")

	for (let i = 0; i < window.Stellium.HotReloadQueue; i++) {
...
function mtHotReload(url) {
	...
	req.onreadystatechange = () => {
		...
		if (req.readyState === 4) {
			...
			const hotContainerElement = document.querySelector('[stellium-hot-container]')
...
if (hotReloadEnabled) {
	...
	let templateEventId = 'mthotreload' + ++window.Stellium.HotReloadQueue
